...
class ScheduleService:
    ...
    def get_schedule(request, filters):
        ...
        movie_sessions_dates = (
            sessions_query.values_list("start_time__date", flat=True)
            .distinct()
        ...
        if not available_dates:
            return 200, {
                "selected_date": date_param or now.date(),
                "available_dates": [],
                "schedule": [],
            }
        ...
        sessions_today = (
            day_sessions.select_related("movie", "hall")
            .order_by("start_time")
        )
        ...
        for s in sessions_today:
            if s.movie_id not in schedule_data:
                schedule_data[s.movie_id] = {
                    "movie": movie_to_dict(s.movie, request),
                    "sessions": [],
                }

            schedule_data[s.movie_id]["sessions"].append(
                {
                    "id": s.id,
